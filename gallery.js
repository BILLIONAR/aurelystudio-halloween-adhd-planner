const inspirationSets = {
 "Costume Planner": {
  "intro": "Find a look you love. Make it comfortable, borrow what you can and add your own twist.",
  "items": [
   [
    "Midnight witch",
    "Velvet layers, a pointed hat and shoes you can enjoy all evening.",
    "Borrow or reuse",
    "Black cape or cardigan|Pointed hat|Comfortable boots|Ribbon or simple accessory",
    "Choose a base outfit|Check fit and movement|Add one finishing detail",
    "assets__ideas__costume-1.png",
    "Costume Planner"
   ],
   [
    "A friendly little ghost",
    "An ivory ghost look with soft shoes and a pumpkin bucket.",
    "Keep it simple",
    "Light ghost cape|Comfortable shoes|Pumpkin bucket|An easy extra layer",
    "Choose a cape with a clear view|Check the length and fit|Try the full outfit before the evening",
    "assets__ideas__costume-2.png",
    "Costume Planner"
   ],
   [
    "The cozy black cat",
    "A familiar black outfit becomes a costume with ears and a playful tail.",
    "Closet first",
    "Black top and trousers|Cat-ear headband|Optional soft tail|Comfortable flats",
    "Start with clothes you already like|Add ears and one detail|Check that everything stays in place",
    "assets__ideas__costume-3.png",
    "Costume Planner"
   ],
   [
    "Moonlit stargazer",
    "Navy fabric, tiny gold stars and a little moonlit sparkle.",
    "A little DIY",
    "Navy cape or shawl|Gold star shapes|Moon hair clips|Simple dark base outfit",
    "Choose your base|Arrange a few stars|Try on the layers together",
    "assets__ideas__costume-4.png",
    "Costume Planner"
   ],
   [
    "Woodland fairy",
    "Leaf details and soft wings bring a woodland idea to familiar clothes.",
    "Soft layers",
    "Comfortable base outfit|Lightweight wings|Leaf accessories",
    "Choose your base|Attach a few leaves|Check fit and movement",
    "assets__ideas__extra-costume-1.png",
    "Costume Planner"
   ],
   [
    "Vintage vampire",
    "A dramatic collar and rich red lining make one strong costume detail.",
    "One statement piece",
    "Dark outfit|Cape|Comfortable shoes",
    "Check your closet|Add a cape or collar|Try the outfit before the evening",
    "assets__ideas__extra-costume-2.png",
    "Costume Planner"
   ],
   [
    "Pumpkin gardener",
    "Overalls, autumn colors and a small pumpkin make a playful seasonal look.",
    "Comfort comes first",
    "Overalls|Warm shirt|Small pumpkin prop",
    "Choose comfortable layers|Add one autumn detail|Check pockets and shoes",
    "assets__ideas__extra-costume-3.png",
    "Costume Planner"
   ],
   [
    "Friendly skeleton",
    "A simple skeleton outfit leaves plenty of room for a cozy layer.",
    "Easy to wear",
    "Skeleton top or costume|Soft trousers|Comfortable shoes",
    "Choose a comfortable fit|Check visibility and movement|Pack an extra layer",
    "assets__ideas__extra-costume-4.png",
    "Costume Planner"
   ],
   [
    "The October detective",
    "A checked coat, a hat and a little mystery from your own wardrobe.",
    "Closet discovery",
    "Coat|Hat|Optional magnifying glass",
    "Choose a base outfit|Borrow one detail|Try on the complete look",
    "assets__ideas__extra-looks-1.png",
    "Costume Planner"
   ],
   [
    "The midnight raven",
    "Dark feathers and a soft cape create a dramatic evening silhouette.",
    "A little drama",
    "Dark base outfit|Soft cape|Feather accessory",
    "Keep the base comfortable|Attach one feather detail|Check fit and movement",
    "assets__ideas__extra-looks-2.png",
    "Costume Planner"
   ]
  ]
 },
 "Shopping List": {
  "intro": "Shop with a purpose. Check what you own, choose quantities and keep each trip small.",
  "items": [
   [
    "Costume essentials",
    "Gather the few pieces that turn your outfit idea into a wearable plan.",
    "Check your closet",
    "Hat or headband|Cape or outer layer|Ribbon|Comfortable shoes",
    "Check what you own|List only the missing pieces|Add sizes, quantities and a shop",
    "assets__ideas__shopping-1.png",
    "Shopping List"
   ],
   [
    "DIY decoration supplies",
    "A small supply kit for a mantel, doorway or cozy corner.",
    "Reuse first",
    "Paper bats|Mini pumpkins|Twine|Battery candles",
    "Choose one space|Check reusable supplies|Decide quantities before buying",
    "assets__ideas__shopping-2.png",
    "Shopping List"
   ],
   [
    "A little hosting kit",
    "The practical basics for snacks, drinks and an easy cleanup.",
    "Count your guests",
    "Plates|Reusable cups|Napkins|Serving bowl",
    "Estimate guests|Check cupboards|Group missing items by shop",
    "assets__ideas__shopping-3.png",
    "Shopping List"
   ],
   [
    "Treat-night top-up",
    "Pick up the treats and little extras you actually need.",
    "One focused trip",
    "Wrapped sweets|Small pencils|Pumpkin buckets|Storage basket",
    "Check what is left at home|Choose quantities|Set a spending limit",
    "assets__ideas__shopping-4.png",
    "Shopping List"
   ],
   [
    "The autumn leaf wreath",
    "A ring of golden leaves makes the doorway feel ready for October.",
    "A handmade welcome",
    "Wreath base|Artificial or dried leaves|Ribbon",
    "Choose a base|Attach leaves securely|Hang it clear of the doorway",
    "assets__ideas__extra-decor-4.png",
    "Shopping List"
   ],
   [
    "Woodland fairy",
    "Leaf details and soft wings bring a woodland idea to familiar clothes.",
    "Soft layers",
    "Comfortable base outfit|Lightweight wings|Leaf accessories",
    "Choose your base|Attach a few leaves|Check fit and movement",
    "assets__ideas__extra-costume-1.png",
    "Shopping List"
   ],
   [
    "Pumpkin-shaped bread rolls",
    "Golden rolls add a handmade autumn detail to your table.",
    "Bake a little",
    "Bread-roll recipe|Dough ingredients|Baking tray",
    "Choose a tested recipe|Check ingredients and timings|Plan a cooling and serving spot",
    "assets__ideas__extra-food-1.png",
    "Shopping List"
   ],
   [
    "See what you already have",
    "A small inventory can make next season’s shopping easier.",
    "Future-you knows",
    "Reusable decorations|Boxes|Notebook",
    "Group similar supplies|Note quantities and condition|Save their storage location",
    "assets__ideas__extra-reset-4.png",
    "Shopping List"
   ],
   [
    "Give each category some room",
    "Plan amounts for the things you want to include in your season.",
    "Plan an amount",
    "Your priorities|Budget categories|A total you choose",
    "Choose a category|Enter a planned amount|Revisit it as your plans change",
    "assets__ideas__extra-money-1.png",
    "Shopping List"
   ],
   [
    "Costume swap afternoon",
    "Share costume pieces and see what can enjoy another October.",
    "Borrow and share",
    "Clean costume pieces|Hangers|Reusable bags",
    "Invite a small group|Sort items by type|Agree which pieces are borrowed or exchanged",
    "assets__ideas__extra-looks-4.png",
    "Shopping List"
   ]
  ]
 },
 "Decorations": {
  "intro": "Choose one small space. These scenes are starting points, ready for your own colors and supplies.",
  "items": [
   [
    "The pumpkin mantel",
    "Mini pumpkins, a bat garland and a warm glow above the fireplace.",
    "One small space",
    "Mini pumpkins|Paper bats|String|Battery candles",
    "Clear a little space|Arrange pumpkins at different heights|Hang the garland",
    "assets__ideas__decorations-1.png",
    "Decorations"
   ],
   [
    "A welcoming doorway",
    "An autumn wreath and a few pumpkin lanterns make the entrance feel special.",
    "A warm welcome",
    "Leaf wreath|Pumpkin lanterns|Porch lights|Removable hooks",
    "Check the doorway space|Place lanterns clear of the walkway|Add the wreath",
    "assets__ideas__decorations-2.png",
    "Decorations"
   ],
   [
    "The cozy reading nook",
    "A soft throw, a ghost cushion and a tiny pumpkin for your quiet corner.",
    "Extra cozy",
    "Plum throw|Ghost cushion|Mini pumpkin|Fairy lights",
    "Choose a chair or corner|Add the soft layers|Arrange one small light",
    "assets__ideas__decorations-3.png",
    "Decorations"
   ],
   [
    "A tiny haunted village",
    "Little glowing houses turn an ordinary shelf into an October scene.",
    "Make a little magic",
    "Cardboard houses|Black card|Battery lights|A clear shelf",
    "Choose the shelf|Make or gather a few houses|Place lights inside and arrange",
    "assets__ideas__decorations-4.png",
    "Decorations"
   ],
   [
    "A floating candle canopy",
    "Suspended battery candles give a small table a magical glow.",
    "Battery lights",
    "Battery candles|Secure supports|Clear table",
    "Choose a small area|Follow mounting instructions|Keep the walkway and serving area clear",
    "assets__ideas__extra-decor-1.png",
    "Decorations"
   ],
   [
    "The autumn apothecary",
    "Amber bottles, dried flowers and tiny pumpkins fill a little shelf.",
    "Gather what you own",
    "Empty decorative bottles|Dried flowers|Mini pumpkins",
    "Choose a stable shelf|Group bottles at different heights|Add a few seasonal details",
    "assets__ideas__extra-decor-2.png",
    "Decorations"
   ],
   [
    "A pumpkin-lit window",
    "Paper bats and glowing pumpkins frame an ordinary window.",
    "One window",
    "Paper bats|Battery pumpkin lights|Removable fixings",
    "Clear the sill|Arrange lights securely|Add a few paper shapes",
    "assets__ideas__extra-decor-3.png",
    "Decorations"
   ],
   [
    "The autumn leaf wreath",
    "A ring of golden leaves makes the doorway feel ready for October.",
    "A handmade welcome",
    "Wreath base|Artificial or dried leaves|Ribbon",
    "Choose a base|Attach leaves securely|Hang it clear of the doorway",
    "assets__ideas__extra-decor-4.png",
    "Decorations"
   ],
   [
    "A night to make something",
    "Set aside a little time for paper bats, a garland or a handmade October detail.",
    "Make a small plan",
    "Paper or card|String|Craft supplies|A clear surface",
    "Choose one small project|Check supplies you already own|Set aside a little making time",
    "assets__ideas__countdown-2.png",
    "Decorations"
   ],
   [
    "Wrap the lights for next year",
    "Wound lights are easier to find and unpack next October.",
    "An easier restart",
    "Lights|Cardboard winders|Storage box",
    "Switch off and unplug|Check the manufacturer’s storage instructions|Wind gently and store dry",
    "assets__ideas__extra-reset-3.png",
    "Decorations"
   ]
  ]
 },
 "Party Planner": {
  "intro": "Pick the kind of gathering you want. A small guest list and one shared activity can be plenty.",
  "items": [
   [
    "Candlelit pumpkin supper",
    "An intimate table with autumn colors and an uncomplicated menu.",
    "Gather around",
    "Pumpkins|Place settings|Battery candles|Simple dinner menu",
    "Set one place for each guest|Arrange pumpkins and leaves with room for serving dishes|Add battery candles clear of plates and glasses",
    "assets__ideas__party-1.png",
    "Party Planner"
   ],
   [
    "Cozy movie night",
    "Cushions, popcorn and one Halloween film everyone can settle into.",
    "A relaxed evening",
    "Film choice|Cushions and blankets|Popcorn|Drinks",
    "Choose a film and test picture and sound|Arrange cushions so everyone can see|Portion popcorn before guests arrive",
    "assets__ideas__party-2.png",
    "Party Planner"
   ],
   [
    "Make-something-spooky party",
    "A shared craft table with simple pumpkin masks and paper bats.",
    "Hands-on fun",
    "Blank masks|Paper|Ribbon|Craft supplies",
    "Clear a work area and set out masks|Cut a few ribbon lengths|Group shared tools in one spot",
    "assets__ideas__party-3.png",
    "Party Planner"
   ],
   [
    "Mocktails & little bites",
    "A colorful drink station with simple snacks and room to mingle.",
    "Mix and mingle",
    "Orange mocktail ingredients|Cups and straws|Cookies|Serving trays",
    "Chill the mocktail before serving|Arrange glasses with space for pouring|Plate cookies and keep food out of pets’ reach",
    "assets__ideas__party-4.png",
    "Party Planner"
   ],
   [
    "A little masquerade",
    "A few decorative masks make a table ready for a playful gathering.",
    "A shared detail",
    "Decorative masks|Ribbon|A clear table",
    "Choose masks that fit comfortably|Arrange a small selection|Leave room for guests’ belongings",
    "assets__ideas__extra-looks-3.png",
    "Party Planner"
   ],
   [
    "Costume swap afternoon",
    "Share costume pieces and see what can enjoy another October.",
    "Borrow and share",
    "Clean costume pieces|Hangers|Reusable bags",
    "Invite a small group|Sort items by type|Agree which pieces are borrowed or exchanged",
    "assets__ideas__extra-looks-4.png",
    "Party Planner"
   ],
   [
    "The hot chocolate corner",
    "Mugs and a small topping selection make a cozy drink station.",
    "Warm up together",
    "Cocoa ingredients|Mugs|Optional toppings",
    "Check ingredients and preferences|Prepare drinks using your recipe|Arrange mugs safely for serving",
    "assets__ideas__extra-sweets-3.png",
    "Party Planner"
   ],
   [
    "Spiced apple cider",
    "Warm apple flavors and cinnamon for a slow autumn evening.",
    "A seasonal sip",
    "Apple juice or cider|Chosen spices|Heat-safe mugs",
    "Choose a non-alcoholic recipe|Check servings and ingredients|Serve at a comfortable temperature",
    "assets__ideas__extra-sweets-4.png",
    "Party Planner"
   ],
   [
    "An October game evening",
    "A small board game and a warm drink make room for shared time.",
    "Play a little",
    "A game you own|Comfortable seats|Drinks",
    "Choose a game together|Clear a little table space|Decide how long you want to play",
    "assets__ideas__extra-season-1.png",
    "Party Planner"
   ],
   [
    "A few puzzle pieces",
    "Enjoy a little autumn puzzle without needing to finish it today.",
    "No rush to finish",
    "Puzzle|A tray or table|Comfortable light",
    "Choose a small area|Put together a few pieces|Leave it ready for another day",
    "assets__ideas__extra-season-2.png",
    "Party Planner"
   ]
  ]
 },
 "Trick or Treat": {
  "intro": "Make a plan for giving out treats, heading out or welcoming everyone home.",
  "items": [
   [
    "The pumpkin treat bowl",
    "A welcoming doorstep setup with wrapped treats ready to share.",
    "Ready at the door",
    "Pumpkin bowl|Wrapped treats|Porch light|A small table",
    "Choose your serving spot|Fill the bowl|Decide your start and finish time",
    "assets__ideas__treat-1.png",
    "Trick or Treat"
   ],
   [
    "Little non-food surprises",
    "Pencils, mini notebooks and pumpkin erasers offer another treat choice.",
    "A little variety",
    "Small pencils|Mini notebooks|Pumpkin erasers|Separate baskets",
    "Choose age-appropriate items|Sort the choices into baskets|Keep a refill nearby",
    "assets__ideas__treat-2.png",
    "Trick or Treat"
   ],
   [
    "Ready for the evening walk",
    "Put your bucket, light and comfortable layers together before leaving.",
    "Pack before you go",
    "Pumpkin bucket|Flashlight|Reflective wristbands|Water and comfortable shoes",
    "Choose your route and meeting point|Set a start and return time|Check lights and pack your bag",
    "assets__ideas__treat-3.png",
    "Trick or Treat"
   ],
   [
    "The cozy welcome home",
    "Warm cocoa, blankets and a place to put everything after the walk.",
    "Wind down",
    "Cocoa ingredients|Mugs|Soft blankets|A place for buckets",
    "Prepare the cozy corner|Set out mugs|Leave tomorrow a small tidy-up plan",
    "assets__ideas__treat-4.png",
    "Trick or Treat"
   ],
   [
    "Make room for the night",
    "Choose the few things that will make your Halloween evening feel like yours.",
    "Your kind of Halloween",
    "Your evening plan|A start time|A few essentials|An easy backup idea",
    "Choose how you want to spend the evening|Check the practical details|Save a date and one next step",
    "assets__ideas__countdown-4.png",
    "Trick or Treat"
   ],
   [
    "Treat-night top-up",
    "Pick up the treats and little extras you actually need.",
    "One focused trip",
    "Wrapped sweets|Small pencils|Pumpkin buckets|Storage basket",
    "Check what is left at home|Choose quantities|Set a spending limit",
    "assets__ideas__shopping-4.png",
    "Trick or Treat"
   ],
   [
    "Vintage vampire",
    "A dramatic collar and rich red lining make one strong costume detail.",
    "One statement piece",
    "Dark outfit|Cape|Comfortable shoes",
    "Check your closet|Add a cape or collar|Try the outfit before the evening",
    "assets__ideas__extra-costume-2.png",
    "Trick or Treat"
   ],
   [
    "Friendly skeleton",
    "A simple skeleton outfit leaves plenty of room for a cozy layer.",
    "Easy to wear",
    "Skeleton top or costume|Soft trousers|Comfortable shoes",
    "Choose a comfortable fit|Check visibility and movement|Pack an extra layer",
    "assets__ideas__extra-costume-4.png",
    "Trick or Treat"
   ],
   [
    "Little ghost meringues",
    "Airy little ghosts for a dessert plate or a shared baking session.",
    "A sweet detail",
    "Meringue recipe|Ingredients|Baking paper",
    "Choose a tested recipe|Check ingredients and dietary needs|Allow the full baking and cooling time",
    "assets__ideas__extra-sweets-1.png",
    "Trick or Treat"
   ],
   [
    "A pumpkin-lit window",
    "Paper bats and glowing pumpkins frame an ordinary window.",
    "One window",
    "Paper bats|Battery pumpkin lights|Removable fixings",
    "Clear the sill|Arrange lights securely|Add a few paper shapes",
    "assets__ideas__extra-decor-3.png",
    "Trick or Treat"
   ]
  ]
 },
 "Meal & Treats": {
  "intro": "Choose something you would enjoy making. Adjust the ingredients, servings and preparation to suit your table.",
  "items": [
   [
    "Pumpkin soup night",
    "A warm bowl of pumpkin soup, toasted bread and a few simple toppings.",
    "A cozy supper",
    "Pumpkin or squash|Vegetable stock|Bread|Pumpkin seeds and herbs",
    "Choose a soup recipe and number of servings|Check ingredients and dietary preferences|Make space for preparation and serving",
    "assets__ideas__meals-1.png",
    "Meal & Treats"
   ],
   [
    "Little broomstick bites",
    "Golden pastry twists and a favorite dip for an easy party platter.",
    "Share a little snack",
    "Ready-made pastry|A dip you enjoy|Baking tray|Serving board",
    "Choose a pastry-twist recipe|Check the pastry baking instructions|Arrange the cooled bites with dip",
    "assets__ideas__meals-2.png",
    "Meal & Treats"
   ],
   [
    "Friendly ghost pretzels",
    "A few dipped pretzels become cheerful little ghosts with chocolate details.",
    "Decorate together",
    "Pretzels|White chocolate or coating|Dark chocolate|Baking paper",
    "Check ingredients and any dietary needs|Dip the pretzels using your chosen recipe|Add little faces and let the coating set",
    "assets__ideas__meals-3.png",
    "Meal & Treats"
   ],
   [
    "Cinnamon apple crumble",
    "Warm apples and a crumbly topping make a simple October dessert.",
    "Something warm",
    "Apples|Cinnamon|Crumble ingredients|Baking dish",
    "Choose a crumble recipe and servings|Check what you already have|Plan baking time and serve when ready",
    "assets__ideas__meals-4.png",
    "Meal & Treats"
   ],
   [
    "Pumpkin-shaped bread rolls",
    "Golden rolls add a handmade autumn detail to your table.",
    "Bake a little",
    "Bread-roll recipe|Dough ingredients|Baking tray",
    "Choose a tested recipe|Check ingredients and timings|Plan a cooling and serving spot",
    "assets__ideas__extra-food-1.png",
    "Meal & Treats"
   ],
   [
    "Little mummy pizzas",
    "Cheese strips and simple toppings turn mini pizzas into playful bites.",
    "A playful supper",
    "Mini pizza bases|Sauce|Cheese|Chosen toppings",
    "Check dietary preferences|Choose a recipe|Follow its cooking instructions",
    "assets__ideas__extra-food-2.png",
    "Meal & Treats"
   ],
   [
    "Stuffed pumpkin peppers",
    "Orange peppers become cheerful bowls for a filling you enjoy.",
    "Make it your own",
    "Orange peppers|Chosen filling|Baking dish",
    "Choose a stuffed-pepper recipe|Check servings and ingredients|Plan preparation and baking time",
    "assets__ideas__extra-food-3.png",
    "Meal & Treats"
   ],
   [
    "Roasted squash pasta",
    "A cozy pasta bowl with autumn squash and your favorite herbs.",
    "A warm meal",
    "Pasta|Squash|Herbs|Chosen sauce",
    "Choose a recipe|Check ingredients and portions|Plan cooking and serving time",
    "assets__ideas__extra-food-4.png",
    "Meal & Treats"
   ],
   [
    "Little ghost meringues",
    "Airy little ghosts for a dessert plate or a shared baking session.",
    "A sweet detail",
    "Meringue recipe|Ingredients|Baking paper",
    "Choose a tested recipe|Check ingredients and dietary needs|Allow the full baking and cooling time",
    "assets__ideas__extra-sweets-1.png",
    "Meal & Treats"
   ],
   [
    "Spiderweb cookies",
    "A simple icing pattern gives favorite cookies a seasonal finish.",
    "Decorate together",
    "Cookies|Icing|Baking paper",
    "Choose cookies you enjoy|Follow an icing recipe|Let the decoration set before storing",
    "assets__ideas__extra-sweets-2.png",
    "Meal & Treats"
   ]
  ]
 },
 "Movie Nights": {
  "intro": "Pick the mood before the movie. Add your own title, where to watch and a night that works for you.",
  "items": [
   [
    "Just-me movie night",
    "An armchair, a favorite blanket and a film chosen entirely for you.",
    "Your own little cinema",
    "Film choice|Blanket|Small popcorn bowl|A drink",
    "Choose a film and where to watch|Set a comfortable start time|Put your snacks within reach",
    "assets__ideas__movies-1.png",
    "Movie Nights"
   ],
   [
    "The blanket-fort screening",
    "Cushions, a little projector and a friendly ghost companion for a playful evening.",
    "Build a cozy den",
    "Blankets|Cushions|Projector or screen|A favorite snack",
    "Arrange a comfortable viewing space|Check the picture and sound|Choose the film together",
    "assets__ideas__movies-2.png",
    "Movie Nights"
   ],
   [
    "A little midnight mystery",
    "Velvet seats, tea and an atmospheric mystery for a slower movie night.",
    "Set the mood",
    "Mystery film choice|Tea or another drink|Comfortable seats|Soft lighting",
    "Choose the film and check its runtime|Arrange seats and drinks|Leave room for a pause",
    "assets__ideas__movies-3.png",
    "Movie Nights"
   ],
   [
    "Under-the-stars cinema",
    "A garden screening with soft lights, blankets and a touch of October.",
    "An outdoor evening",
    "Screen or projector|Garden chairs|Blankets|Outdoor lights",
    "Check the weather and choose a backup spot|Test the setup before dark|Plan a start time and warm layers",
    "assets__ideas__movies-4.png",
    "Movie Nights"
   ],
   [
    "Cozy movie night",
    "Cushions, popcorn and one Halloween film everyone can settle into.",
    "A relaxed evening",
    "Film choice|Cushions and blankets|Popcorn|Drinks",
    "Choose a film and test picture and sound|Arrange cushions so everyone can see|Portion popcorn before guests arrive",
    "assets__ideas__party-2.png",
    "Movie Nights"
   ],
   [
    "Mocktails & little bites",
    "A colorful drink station with simple snacks and room to mingle.",
    "Mix and mingle",
    "Orange mocktail ingredients|Cups and straws|Cookies|Serving trays",
    "Chill the mocktail before serving|Arrange glasses with space for pouring|Plate cookies and keep food out of pets’ reach",
    "assets__ideas__party-4.png",
    "Movie Nights"
   ],
   [
    "The hot chocolate corner",
    "Mugs and a small topping selection make a cozy drink station.",
    "Warm up together",
    "Cocoa ingredients|Mugs|Optional toppings",
    "Check ingredients and preferences|Prepare drinks using your recipe|Arrange mugs safely for serving",
    "assets__ideas__extra-sweets-3.png",
    "Movie Nights"
   ],
   [
    "Spiced apple cider",
    "Warm apple flavors and cinnamon for a slow autumn evening.",
    "A seasonal sip",
    "Apple juice or cider|Chosen spices|Heat-safe mugs",
    "Choose a non-alcoholic recipe|Check servings and ingredients|Serve at a comfortable temperature",
    "assets__ideas__extra-sweets-4.png",
    "Movie Nights"
   ],
   [
    "A few puzzle pieces",
    "Enjoy a little autumn puzzle without needing to finish it today.",
    "No rush to finish",
    "Puzzle|A tray or table|Comfortable light",
    "Choose a small area|Put together a few pieces|Leave it ready for another day",
    "assets__ideas__extra-season-2.png",
    "Movie Nights"
   ],
   [
    "Listen to a favorite sound",
    "A comfortable seat and a familiar playlist for a small pause.",
    "Your own soundtrack",
    "Headphones or speaker|Music you enjoy|Comfortable seat",
    "Choose something you enjoy|Set a comfortable volume|Pause whenever you want",
    "assets__ideas__extra-season-3.png",
    "Movie Nights"
   ]
  ]
 },
 "Self-Care": {
  "intro": "A few gentle ways to enjoy your season. Choose what appeals to you and leave the rest.",
  "items": [
   [
    "Tea by the window",
    "A warm cup and a quiet window seat for a moment with no agenda.",
    "A small pause",
    "A drink you enjoy|Favorite mug|Comfortable seat|Optional soft blanket",
    "Make your drink|Choose somewhere comfortable|Stay for as little or as long as you like",
    "assets__ideas__care-1.png",
    "Self-Care"
   ],
   [
    "A few pages, just for you",
    "A book, a blanket and a softly lit corner to enjoy at your own pace.",
    "Read a little",
    "A book you like|Reading light|Blanket|Bookmark",
    "Choose a book that appeals to you|Settle into your corner|Stop wherever feels right",
    "assets__ideas__care-2.png",
    "Self-Care"
   ],
   [
    "A little autumn journaling",
    "A blank page and a pen, with room for a thought, a doodle or nothing much.",
    "No perfect words",
    "Notebook|Pen|Optional colored pencils|A clear little space",
    "Open a fresh page|Write one thought or draw one shape|Leave the page as it is",
    "assets__ideas__care-3.png",
    "Self-Care"
   ],
   [
    "An autumn color walk",
    "A short walk for noticing golden leaves, quiet corners and the changing season.",
    "Enjoy what you notice",
    "Comfortable shoes|Weather-appropriate layer|Optional water bottle|A familiar route",
    "Choose a route and check the weather|Go at a pace you enjoy|Notice one color or detail along the way",
    "assets__ideas__care-4.png",
    "Self-Care"
   ],
   [
    "One clear patch of desk",
    "A little working space with just the things your next step needs.",
    "Make room to begin",
    "Your next task|The supplies it needs|A clear patch of desk|A drink if you like",
    "Choose one next step|Move unrelated items aside|Pick a timer length when you are ready",
    "assets__ideas__focus-1.png",
    "Self-Care"
   ],
   [
    "A small session, made visible",
    "An hourglass-inspired reminder that a session can have a clear beginning and end.",
    "Choose your time",
    "One task|The app timer|A comfortable seat|A stopping point",
    "Choose what you will work on|Set a timer length that suits you|Decide what you will do when it ends",
    "assets__ideas__focus-2.png",
    "Self-Care"
   ],
   [
    "A quiet reading session",
    "A book, a reading light and enough space to settle into one small piece.",
    "One page is a start",
    "Book or reading material|Reading light|Bookmark|A comfortable spot",
    "Choose a page or section|Settle into your space|Use the timer if it feels useful",
    "assets__ideas__focus-3.png",
    "Self-Care"
   ],
   [
    "Leave a place for the next step",
    "A small finishing ritual to make it easier to return to your task later.",
    "A gentle stopping point",
    "Your task notes|A pen|A place for supplies|A note for next time",
    "Notice where you stopped|Write the next small step|Put the essentials together for later",
    "assets__ideas__focus-4.png",
    "Self-Care"
   ],
   [
    "Listen to a favorite sound",
    "A comfortable seat and a familiar playlist for a small pause.",
    "Your own soundtrack",
    "Headphones or speaker|Music you enjoy|Comfortable seat",
    "Choose something you enjoy|Set a comfortable volume|Pause whenever you want",
    "assets__ideas__extra-season-3.png",
    "Self-Care"
   ],
   [
    "Make a little room to move",
    "A clear corner and a soft mat invite a comfortable movement break.",
    "A gentle pause",
    "Clear space|Optional mat|Water",
    "Choose movements comfortable for you|Go at your own pace|Stop when you want",
    "assets__ideas__extra-season-4.png",
    "Self-Care"
   ]
  ]
 },
 "After Halloween": {
  "intro": "Keep what you loved, put away a little at a time and leave next October a few helpful notes.",
  "items": [
   [
    "Pack away the pumpkins",
    "Store the decorations you want to see again, grouped so they are easy to find.",
    "A little at a time",
    "Storage box|Tissue or wrapping|Labels|Decorations to keep",
    "Choose one small area|Wrap fragile pieces and group items|Label the box and note its storage spot",
    "assets__ideas__after-1.png",
    "After Halloween"
   ],
   [
    "Keep, share or let go",
    "A small sorting session for costumes and decorations you no longer need.",
    "Make a little room",
    "Baskets or boxes|Reusable bags|A short inventory|A place to sort",
    "Choose a manageable group of items|Separate what to keep and what to pass on|Decide the next step for each group",
    "assets__ideas__after-2.png",
    "After Halloween"
   ],
   [
    "Save the little memories",
    "A few photos and keepsakes are enough to remember your favorite moments.",
    "Keep the good bits",
    "Favorite photos|Small keepsakes|Notebook or album|Pen",
    "Choose a few moments|Add a short note about each|Put them somewhere you can find again",
    "assets__ideas__after-3.png",
    "After Halloween"
   ],
   [
    "A note for next October",
    "Leave yourself the ideas, favorites and practical details worth remembering.",
    "Future-you will know",
    "Notebook|Pen|Your saved plans|Storage notes",
    "Write what you enjoyed|Note what you would change|Keep useful quantities and storage locations",
    "assets__ideas__after-4.png",
    "After Halloween"
   ],
   [
    "Refresh the costume pieces",
    "Clean and air your costumes before putting them away.",
    "Check care labels",
    "Costume pieces|Hangers|Care instructions",
    "Read each care label|Clean as directed|Dry fully before storing",
    "assets__ideas__extra-reset-1.png",
    "After Halloween"
   ],
   [
    "Repair a favorite decoration",
    "Fix one little detail so a favorite piece is ready to return.",
    "One small repair",
    "Decoration|Suitable repair supplies|Clear work surface",
    "Check what needs attention|Choose suitable supplies|Repair and allow to set",
    "assets__ideas__extra-reset-2.png",
    "After Halloween"
   ],
   [
    "Wrap the lights for next year",
    "Wound lights are easier to find and unpack next October.",
    "An easier restart",
    "Lights|Cardboard winders|Storage box",
    "Switch off and unplug|Check the manufacturer’s storage instructions|Wind gently and store dry",
    "assets__ideas__extra-reset-3.png",
    "After Halloween"
   ],
   [
    "See what you already have",
    "A small inventory can make next season’s shopping easier.",
    "Future-you knows",
    "Reusable decorations|Boxes|Notebook",
    "Group similar supplies|Note quantities and condition|Save their storage location",
    "assets__ideas__extra-reset-4.png",
    "After Halloween"
   ],
   [
    "Leave a place for the next step",
    "A small finishing ritual to make it easier to return to your task later.",
    "A gentle stopping point",
    "Your task notes|A pen|A place for supplies|A note for next time",
    "Notice where you stopped|Write the next small step|Put the essentials together for later",
    "assets__ideas__focus-4.png",
    "After Halloween"
   ],
   [
    "Save it for another day",
    "A place for ideas you want to keep without acting on them today.",
    "It can wait",
    "Journal or planner|Loose thoughts|A pen|Some room for later",
    "Write the idea down|Note anything you might want to remember|Leave the next step for another day",
    "assets__ideas__brain-4.png",
    "After Halloween"
   ]
  ]
 },
 "Brain Dump": {
  "intro": "Give a thought somewhere to land. Pick a prompt, change it freely and save only what you want.",
  "items": [
   [
    "Let the thoughts land",
    "A blank page for whatever is taking up space in your head.",
    "No need to organize",
    "Notebook or planner|A pen|A place to sit|One thought to begin",
    "Write whatever comes to mind|Leave the order as it is|Circle one thing only if you want",
    "assets__ideas__brain-1.png",
    "Brain Dump"
   ],
   [
    "One thought at a time",
    "Separate little notes can make it easier to see what you have written.",
    "Keep it small",
    "Loose notes|Pen or pencils|A small bowl or tray|Your planner",
    "Put one thought on each note|Gather the notes together|Choose what to keep in your planner",
    "assets__ideas__brain-2.png",
    "Brain Dump"
   ],
   [
    "A place for October ideas",
    "Collect the colors, costumes and small seasonal plans that catch your attention.",
    "Follow your curiosity",
    "Blank notes|A few sketches|A board or notebook|Your favorite ideas",
    "Capture an idea without planning it all|Add a sketch or a few words|Return to it when you feel ready",
    "assets__ideas__brain-3.png",
    "Brain Dump"
   ],
   [
    "Save it for another day",
    "A place for ideas you want to keep without acting on them today.",
    "It can wait",
    "Journal or planner|Loose thoughts|A pen|Some room for later",
    "Write the idea down|Note anything you might want to remember|Leave the next step for another day",
    "assets__ideas__brain-4.png",
    "Brain Dump"
   ],
   [
    "One clear patch of desk",
    "A little working space with just the things your next step needs.",
    "Make room to begin",
    "Your next task|The supplies it needs|A clear patch of desk|A drink if you like",
    "Choose one next step|Move unrelated items aside|Pick a timer length when you are ready",
    "assets__ideas__focus-1.png",
    "Brain Dump"
   ],
   [
    "A small session, made visible",
    "An hourglass-inspired reminder that a session can have a clear beginning and end.",
    "Choose your time",
    "One task|The app timer|A comfortable seat|A stopping point",
    "Choose what you will work on|Set a timer length that suits you|Decide what you will do when it ends",
    "assets__ideas__focus-2.png",
    "Brain Dump"
   ],
   [
    "A quiet reading session",
    "A book, a reading light and enough space to settle into one small piece.",
    "One page is a start",
    "Book or reading material|Reading light|Bookmark|A comfortable spot",
    "Choose a page or section|Settle into your space|Use the timer if it feels useful",
    "assets__ideas__focus-3.png",
    "Brain Dump"
   ],
   [
    "Leave a place for the next step",
    "A small finishing ritual to make it easier to return to your task later.",
    "A gentle stopping point",
    "Your task notes|A pen|A place for supplies|A note for next time",
    "Notice where you stopped|Write the next small step|Put the essentials together for later",
    "assets__ideas__focus-4.png",
    "Brain Dump"
   ],
   [
    "A little autumn journaling",
    "A blank page and a pen, with room for a thought, a doodle or nothing much.",
    "No perfect words",
    "Notebook|Pen|Optional colored pencils|A clear little space",
    "Open a fresh page|Write one thought or draw one shape|Leave the page as it is",
    "assets__ideas__care-3.png",
    "Brain Dump"
   ],
   [
    "Compare before you choose",
    "Keep a few options together before deciding where to spend.",
    "A considered choice",
    "Two or three options|Prices you have checked|Your spending limit",
    "Compare like-for-like items|Include delivery or extra costs|Save the amount you choose",
    "assets__ideas__extra-money-4.png",
    "Brain Dump"
   ]
  ]
 },
 "Focus Tools": {
  "intro": "Prepare a space for one small session. These ideas save as editable tasks; your timer stays yours to start.",
  "items": [
   [
    "One clear patch of desk",
    "A little working space with just the things your next step needs.",
    "Make room to begin",
    "Your next task|The supplies it needs|A clear patch of desk|A drink if you like",
    "Choose one next step|Move unrelated items aside|Pick a timer length when you are ready",
    "assets__ideas__focus-1.png",
    "Task Planner"
   ],
   [
    "A small session, made visible",
    "An hourglass-inspired reminder that a session can have a clear beginning and end.",
    "Choose your time",
    "One task|The app timer|A comfortable seat|A stopping point",
    "Choose what you will work on|Set a timer length that suits you|Decide what you will do when it ends",
    "assets__ideas__focus-2.png",
    "Task Planner"
   ],
   [
    "A quiet reading session",
    "A book, a reading light and enough space to settle into one small piece.",
    "One page is a start",
    "Book or reading material|Reading light|Bookmark|A comfortable spot",
    "Choose a page or section|Settle into your space|Use the timer if it feels useful",
    "assets__ideas__focus-3.png",
    "Task Planner"
   ],
   [
    "Leave a place for the next step",
    "A small finishing ritual to make it easier to return to your task later.",
    "A gentle stopping point",
    "Your task notes|A pen|A place for supplies|A note for next time",
    "Notice where you stopped|Write the next small step|Put the essentials together for later",
    "assets__ideas__focus-4.png",
    "Task Planner"
   ],
   [
    "One thought at a time",
    "Separate little notes can make it easier to see what you have written.",
    "Keep it small",
    "Loose notes|Pen or pencils|A small bowl or tray|Your planner",
    "Put one thought on each note|Gather the notes together|Choose what to keep in your planner",
    "assets__ideas__brain-2.png",
    "Task Planner"
   ],
   [
    "Save it for another day",
    "A place for ideas you want to keep without acting on them today.",
    "It can wait",
    "Journal or planner|Loose thoughts|A pen|Some room for later",
    "Write the idea down|Note anything you might want to remember|Leave the next step for another day",
    "assets__ideas__brain-4.png",
    "Task Planner"
   ],
   [
    "Tea by the window",
    "A warm cup and a quiet window seat for a moment with no agenda.",
    "A small pause",
    "A drink you enjoy|Favorite mug|Comfortable seat|Optional soft blanket",
    "Make your drink|Choose somewhere comfortable|Stay for as little or as long as you like",
    "assets__ideas__care-1.png",
    "Task Planner"
   ],
   [
    "A few pages, just for you",
    "A book, a blanket and a softly lit corner to enjoy at your own pace.",
    "Read a little",
    "A book you like|Reading light|Blanket|Bookmark",
    "Choose a book that appeals to you|Settle into your corner|Stop wherever feels right",
    "assets__ideas__care-2.png",
    "Task Planner"
   ],
   [
    "Listen to a favorite sound",
    "A comfortable seat and a familiar playlist for a small pause.",
    "Your own soundtrack",
    "Headphones or speaker|Music you enjoy|Comfortable seat",
    "Choose something you enjoy|Set a comfortable volume|Pause whenever you want",
    "assets__ideas__extra-season-3.png",
    "Task Planner"
   ],
   [
    "Make a little room to move",
    "A clear corner and a soft mat invite a comfortable movement break.",
    "A gentle pause",
    "Clear space|Optional mat|Water",
    "Choose movements comfortable for you|Go at your own pace|Stop when you want",
    "assets__ideas__extra-season-4.png",
    "Task Planner"
   ]
  ]
 },
 "Halloween Countdown": {
  "intro": "Choose a seasonal milestone to look forward to. Save it in Task Planner with a date that suits you.",
  "items": [
   [
    "Choose your pumpkin",
    "A visit to a pumpkin patch or a little pumpkin picked up close to home.",
    "One autumn outing",
    "A place to visit|A date that works|A way to carry your pumpkin|An optional companion",
    "Choose where you would like to go|Check opening times before leaving|Give the outing a date",
    "assets__ideas__countdown-1.png",
    "Task Planner"
   ],
   [
    "A night to make something",
    "Set aside a little time for paper bats, a garland or a handmade October detail.",
    "Make a small plan",
    "Paper or card|String|Craft supplies|A clear surface",
    "Choose one small project|Check supplies you already own|Set aside a little making time",
    "assets__ideas__countdown-2.png",
    "Task Planner"
   ],
   [
    "Try the costume together",
    "Give your outfit a simple rehearsal so the details are ready when you need them.",
    "A little preparation",
    "Your outfit|Shoes|Accessories|A mirror",
    "Gather your pieces|Try them together|Write down any small adjustments",
    "assets__ideas__countdown-3.png",
    "Task Planner"
   ],
   [
    "Make room for the night",
    "Choose the few things that will make your Halloween evening feel like yours.",
    "Your kind of Halloween",
    "Your evening plan|A start time|A few essentials|An easy backup idea",
    "Choose how you want to spend the evening|Check the practical details|Save a date and one next step",
    "assets__ideas__countdown-4.png",
    "Task Planner"
   ],
   [
    "Woodland fairy",
    "Leaf details and soft wings bring a woodland idea to familiar clothes.",
    "Soft layers",
    "Comfortable base outfit|Lightweight wings|Leaf accessories",
    "Choose your base|Attach a few leaves|Check fit and movement",
    "assets__ideas__extra-costume-1.png",
    "Task Planner"
   ],
   [
    "The autumn leaf wreath",
    "A ring of golden leaves makes the doorway feel ready for October.",
    "A handmade welcome",
    "Wreath base|Artificial or dried leaves|Ribbon",
    "Choose a base|Attach leaves securely|Hang it clear of the doorway",
    "assets__ideas__extra-decor-4.png",
    "Task Planner"
   ],
   [
    "Make-something-spooky party",
    "A shared craft table with simple pumpkin masks and paper bats.",
    "Hands-on fun",
    "Blank masks|Paper|Ribbon|Craft supplies",
    "Clear a work area and set out masks|Cut a few ribbon lengths|Group shared tools in one spot",
    "assets__ideas__party-3.png",
    "Task Planner"
   ],
   [
    "Pumpkin-shaped bread rolls",
    "Golden rolls add a handmade autumn detail to your table.",
    "Bake a little",
    "Bread-roll recipe|Dough ingredients|Baking tray",
    "Choose a tested recipe|Check ingredients and timings|Plan a cooling and serving spot",
    "assets__ideas__extra-food-1.png",
    "Task Planner"
   ],
   [
    "An October game evening",
    "A small board game and a warm drink make room for shared time.",
    "Play a little",
    "A game you own|Comfortable seats|Drinks",
    "Choose a game together|Clear a little table space|Decide how long you want to play",
    "assets__ideas__extra-season-1.png",
    "Task Planner"
   ],
   [
    "Pack away the pumpkins",
    "Store the decorations you want to see again, grouped so they are easy to find.",
    "A little at a time",
    "Storage box|Tissue or wrapping|Labels|Decorations to keep",
    "Choose one small area|Wrap fragile pieces and group items|Label the box and note its storage spot",
    "assets__ideas__after-1.png",
    "Task Planner"
   ]
  ]
 },
 "Budget": {
  "intro": "Use these scenes to plan categories, compare options and record your own amounts.",
  "items": [
   [
    "Give each category some room",
    "Plan amounts for the things you want to include in your season.",
    "Plan an amount",
    "Your priorities|Budget categories|A total you choose",
    "Choose a category|Enter a planned amount|Revisit it as your plans change",
    "assets__ideas__extra-money-1.png",
    "Budget"
   ],
   [
    "A place for the receipts",
    "Gather purchase details so actual spending is easier to record.",
    "Record as you go",
    "Receipts|A small tray|Your planner",
    "Gather recent receipts|Enter actual amounts|Check each entry before saving",
    "assets__ideas__extra-money-2.png",
    "Budget"
   ],
   [
    "Shop your saved supplies",
    "Take a look at reusable pieces before planning another purchase.",
    "Use what you have",
    "Saved decorations|Costume accessories|Inventory notes",
    "Choose one storage box|Check condition and quantity|Budget only for what is missing",
    "assets__ideas__extra-money-3.png",
    "Budget"
   ],
   [
    "Compare before you choose",
    "Keep a few options together before deciding where to spend.",
    "A considered choice",
    "Two or three options|Prices you have checked|Your spending limit",
    "Compare like-for-like items|Include delivery or extra costs|Save the amount you choose",
    "assets__ideas__extra-money-4.png",
    "Budget"
   ],
   [
    "Costume essentials",
    "Gather the few pieces that turn your outfit idea into a wearable plan.",
    "Check your closet",
    "Hat or headband|Cape or outer layer|Ribbon|Comfortable shoes",
    "Check what you own|List only the missing pieces|Add sizes, quantities and a shop",
    "assets__ideas__shopping-1.png",
    "Budget"
   ],
   [
    "DIY decoration supplies",
    "A small supply kit for a mantel, doorway or cozy corner.",
    "Reuse first",
    "Paper bats|Mini pumpkins|Twine|Battery candles",
    "Choose one space|Check reusable supplies|Decide quantities before buying",
    "assets__ideas__shopping-2.png",
    "Budget"
   ],
   [
    "A little hosting kit",
    "The practical basics for snacks, drinks and an easy cleanup.",
    "Count your guests",
    "Plates|Reusable cups|Napkins|Serving bowl",
    "Estimate guests|Check cupboards|Group missing items by shop",
    "assets__ideas__shopping-3.png",
    "Budget"
   ],
   [
    "Treat-night top-up",
    "Pick up the treats and little extras you actually need.",
    "One focused trip",
    "Wrapped sweets|Small pencils|Pumpkin buckets|Storage basket",
    "Check what is left at home|Choose quantities|Set a spending limit",
    "assets__ideas__shopping-4.png",
    "Budget"
   ],
   [
    "See what you already have",
    "A small inventory can make next season’s shopping easier.",
    "Future-you knows",
    "Reusable decorations|Boxes|Notebook",
    "Group similar supplies|Note quantities and condition|Save their storage location",
    "assets__ideas__extra-reset-4.png",
    "Budget"
   ],
   [
    "Costume swap afternoon",
    "Share costume pieces and see what can enjoy another October.",
    "Borrow and share",
    "Clean costume pieces|Hangers|Reusable bags",
    "Invite a small group|Sort items by type|Agree which pieces are borrowed or exchanged",
    "assets__ideas__extra-looks-4.png",
    "Budget"
   ]
  ]
 },
 "Dashboard": {
  "intro": "Explore a little of your season. Each idea opens a draft in its matching planner.",
  "items": [
   [
    "Woodland fairy",
    "Leaf details and soft wings bring a woodland idea to familiar clothes.",
    "Soft layers",
    "Comfortable base outfit|Lightweight wings|Leaf accessories",
    "Choose your base|Attach a few leaves|Check fit and movement",
    "assets__ideas__extra-costume-1.png",
    "Costume Planner"
   ],
   [
    "The pumpkin mantel",
    "Mini pumpkins, a bat garland and a warm glow above the fireplace.",
    "One small space",
    "Mini pumpkins|Paper bats|String|Battery candles",
    "Clear a little space|Arrange pumpkins at different heights|Hang the garland",
    "assets__ideas__decorations-1.png",
    "Decorations"
   ],
   [
    "Candlelit pumpkin supper",
    "An intimate table with autumn colors and an uncomplicated menu.",
    "Gather around",
    "Pumpkins|Place settings|Battery candles|Simple dinner menu",
    "Set one place for each guest|Arrange pumpkins and leaves with room for serving dishes|Add battery candles clear of plates and glasses",
    "assets__ideas__party-1.png",
    "Party Planner"
   ],
   [
    "Ready for the evening walk",
    "Put your bucket, light and comfortable layers together before leaving.",
    "Pack before you go",
    "Pumpkin bucket|Flashlight|Reflective wristbands|Water and comfortable shoes",
    "Choose your route and meeting point|Set a start and return time|Check lights and pack your bag",
    "assets__ideas__treat-3.png",
    "Trick or Treat"
   ],
   [
    "Pumpkin soup night",
    "A warm bowl of pumpkin soup, toasted bread and a few simple toppings.",
    "A cozy supper",
    "Pumpkin or squash|Vegetable stock|Bread|Pumpkin seeds and herbs",
    "Choose a soup recipe and number of servings|Check ingredients and dietary preferences|Make space for preparation and serving",
    "assets__ideas__meals-1.png",
    "Meal & Treats"
   ],
   [
    "Just-me movie night",
    "An armchair, a favorite blanket and a film chosen entirely for you.",
    "Your own little cinema",
    "Film choice|Blanket|Small popcorn bowl|A drink",
    "Choose a film and where to watch|Set a comfortable start time|Put your snacks within reach",
    "assets__ideas__movies-1.png",
    "Movie Nights"
   ],
   [
    "Tea by the window",
    "A warm cup and a quiet window seat for a moment with no agenda.",
    "A small pause",
    "A drink you enjoy|Favorite mug|Comfortable seat|Optional soft blanket",
    "Make your drink|Choose somewhere comfortable|Stay for as little or as long as you like",
    "assets__ideas__care-1.png",
    "Self-Care"
   ],
   [
    "Let the thoughts land",
    "A blank page for whatever is taking up space in your head.",
    "No need to organize",
    "Notebook or planner|A pen|A place to sit|One thought to begin",
    "Write whatever comes to mind|Leave the order as it is|Circle one thing only if you want",
    "assets__ideas__brain-1.png",
    "Brain Dump"
   ],
   [
    "One clear patch of desk",
    "A little working space with just the things your next step needs.",
    "Make room to begin",
    "Your next task|The supplies it needs|A clear patch of desk|A drink if you like",
    "Choose one next step|Move unrelated items aside|Pick a timer length when you are ready",
    "assets__ideas__focus-1.png",
    "Task Planner"
   ],
   [
    "Pack away the pumpkins",
    "Store the decorations you want to see again, grouped so they are easy to find.",
    "A little at a time",
    "Storage box|Tissue or wrapping|Labels|Decorations to keep",
    "Choose one small area|Wrap fragile pieces and group items|Label the box and note its storage spot",
    "assets__ideas__after-1.png",
    "After Halloween"
   ]
  ]
 },
 "Today": {
  "intro": "Choose one small idea for today. You can save it as a task when you are ready.",
  "items": [
   [
    "One clear patch of desk",
    "A little working space with just the things your next step needs.",
    "Make room to begin",
    "Your next task|The supplies it needs|A clear patch of desk|A drink if you like",
    "Choose one next step|Move unrelated items aside|Pick a timer length when you are ready",
    "assets__ideas__focus-1.png",
    "Task Planner"
   ],
   [
    "One thought at a time",
    "Separate little notes can make it easier to see what you have written.",
    "Keep it small",
    "Loose notes|Pen or pencils|A small bowl or tray|Your planner",
    "Put one thought on each note|Gather the notes together|Choose what to keep in your planner",
    "assets__ideas__brain-2.png",
    "Task Planner"
   ],
   [
    "Tea by the window",
    "A warm cup and a quiet window seat for a moment with no agenda.",
    "A small pause",
    "A drink you enjoy|Favorite mug|Comfortable seat|Optional soft blanket",
    "Make your drink|Choose somewhere comfortable|Stay for as little or as long as you like",
    "assets__ideas__care-1.png",
    "Task Planner"
   ],
   [
    "Costume essentials",
    "Gather the few pieces that turn your outfit idea into a wearable plan.",
    "Check your closet",
    "Hat or headband|Cape or outer layer|Ribbon|Comfortable shoes",
    "Check what you own|List only the missing pieces|Add sizes, quantities and a shop",
    "assets__ideas__shopping-1.png",
    "Task Planner"
   ],
   [
    "A night to make something",
    "Set aside a little time for paper bats, a garland or a handmade October detail.",
    "Make a small plan",
    "Paper or card|String|Craft supplies|A clear surface",
    "Choose one small project|Check supplies you already own|Set aside a little making time",
    "assets__ideas__countdown-2.png",
    "Task Planner"
   ],
   [
    "The hot chocolate corner",
    "Mugs and a small topping selection make a cozy drink station.",
    "Warm up together",
    "Cocoa ingredients|Mugs|Optional toppings",
    "Check ingredients and preferences|Prepare drinks using your recipe|Arrange mugs safely for serving",
    "assets__ideas__extra-sweets-3.png",
    "Task Planner"
   ],
   [
    "An autumn color walk",
    "A short walk for noticing golden leaves, quiet corners and the changing season.",
    "Enjoy what you notice",
    "Comfortable shoes|Weather-appropriate layer|Optional water bottle|A familiar route",
    "Choose a route and check the weather|Go at a pace you enjoy|Notice one color or detail along the way",
    "assets__ideas__care-4.png",
    "Task Planner"
   ],
   [
    "Make a little room to move",
    "A clear corner and a soft mat invite a comfortable movement break.",
    "A gentle pause",
    "Clear space|Optional mat|Water",
    "Choose movements comfortable for you|Go at your own pace|Stop when you want",
    "assets__ideas__extra-season-4.png",
    "Task Planner"
   ],
   [
    "Wrap the lights for next year",
    "Wound lights are easier to find and unpack next October.",
    "An easier restart",
    "Lights|Cardboard winders|Storage box",
    "Switch off and unplug|Check the manufacturer’s storage instructions|Wind gently and store dry",
    "assets__ideas__extra-reset-3.png",
    "Task Planner"
   ],
   [
    "Leave a place for the next step",
    "A small finishing ritual to make it easier to return to your task later.",
    "A gentle stopping point",
    "Your task notes|A pen|A place for supplies|A note for next time",
    "Notice where you stopped|Write the next small step|Put the essentials together for later",
    "assets__ideas__focus-4.png",
    "Task Planner"
   ]
  ]
 },
 "Task Planner": {
  "intro": "Turn one idea into a small next step, with a date and details of your own.",
  "items": [
   [
    "Choose your pumpkin",
    "A visit to a pumpkin patch or a little pumpkin picked up close to home.",
    "One autumn outing",
    "A place to visit|A date that works|A way to carry your pumpkin|An optional companion",
    "Choose where you would like to go|Check opening times before leaving|Give the outing a date",
    "assets__ideas__countdown-1.png",
    "Task Planner"
   ],
   [
    "A night to make something",
    "Set aside a little time for paper bats, a garland or a handmade October detail.",
    "Make a small plan",
    "Paper or card|String|Craft supplies|A clear surface",
    "Choose one small project|Check supplies you already own|Set aside a little making time",
    "assets__ideas__countdown-2.png",
    "Task Planner"
   ],
   [
    "Try the costume together",
    "Give your outfit a simple rehearsal so the details are ready when you need them.",
    "A little preparation",
    "Your outfit|Shoes|Accessories|A mirror",
    "Gather your pieces|Try them together|Write down any small adjustments",
    "assets__ideas__countdown-3.png",
    "Task Planner"
   ],
   [
    "Make room for the night",
    "Choose the few things that will make your Halloween evening feel like yours.",
    "Your kind of Halloween",
    "Your evening plan|A start time|A few essentials|An easy backup idea",
    "Choose how you want to spend the evening|Check the practical details|Save a date and one next step",
    "assets__ideas__countdown-4.png",
    "Task Planner"
   ],
   [
    "DIY decoration supplies",
    "A small supply kit for a mantel, doorway or cozy corner.",
    "Reuse first",
    "Paper bats|Mini pumpkins|Twine|Battery candles",
    "Choose one space|Check reusable supplies|Decide quantities before buying",
    "assets__ideas__shopping-2.png",
    "Task Planner"
   ],
   [
    "Make-something-spooky party",
    "A shared craft table with simple pumpkin masks and paper bats.",
    "Hands-on fun",
    "Blank masks|Paper|Ribbon|Craft supplies",
    "Clear a work area and set out masks|Cut a few ribbon lengths|Group shared tools in one spot",
    "assets__ideas__party-3.png",
    "Task Planner"
   ],
   [
    "Repair a favorite decoration",
    "Fix one little detail so a favorite piece is ready to return.",
    "One small repair",
    "Decoration|Suitable repair supplies|Clear work surface",
    "Check what needs attention|Choose suitable supplies|Repair and allow to set",
    "assets__ideas__extra-reset-2.png",
    "Task Planner"
   ],
   [
    "See what you already have",
    "A small inventory can make next season’s shopping easier.",
    "Future-you knows",
    "Reusable decorations|Boxes|Notebook",
    "Group similar supplies|Note quantities and condition|Save their storage location",
    "assets__ideas__extra-reset-4.png",
    "Task Planner"
   ],
   [
    "One clear patch of desk",
    "A little working space with just the things your next step needs.",
    "Make room to begin",
    "Your next task|The supplies it needs|A clear patch of desk|A drink if you like",
    "Choose one next step|Move unrelated items aside|Pick a timer length when you are ready",
    "assets__ideas__focus-1.png",
    "Task Planner"
   ],
   [
    "Leave a place for the next step",
    "A small finishing ritual to make it easier to return to your task later.",
    "A gentle stopping point",
    "Your task notes|A pen|A place for supplies|A note for next time",
    "Notice where you stopped|Write the next small step|Put the essentials together for later",
    "assets__ideas__focus-4.png",
    "Task Planner"
   ]
  ]
 },
 "Overwhelmed Mode": {
  "intro": "These ideas are optional. One small pause or step can be enough for now.",
  "items": [
   [
    "Tea by the window",
    "A warm cup and a quiet window seat for a moment with no agenda.",
    "A small pause",
    "A drink you enjoy|Favorite mug|Comfortable seat|Optional soft blanket",
    "Make your drink|Choose somewhere comfortable|Stay for as little or as long as you like",
    "assets__ideas__care-1.png",
    "Task Planner"
   ],
   [
    "A few pages, just for you",
    "A book, a blanket and a softly lit corner to enjoy at your own pace.",
    "Read a little",
    "A book you like|Reading light|Blanket|Bookmark",
    "Choose a book that appeals to you|Settle into your corner|Stop wherever feels right",
    "assets__ideas__care-2.png",
    "Task Planner"
   ],
   [
    "One thought at a time",
    "Separate little notes can make it easier to see what you have written.",
    "Keep it small",
    "Loose notes|Pen or pencils|A small bowl or tray|Your planner",
    "Put one thought on each note|Gather the notes together|Choose what to keep in your planner",
    "assets__ideas__brain-2.png",
    "Task Planner"
   ],
   [
    "One clear patch of desk",
    "A little working space with just the things your next step needs.",
    "Make room to begin",
    "Your next task|The supplies it needs|A clear patch of desk|A drink if you like",
    "Choose one next step|Move unrelated items aside|Pick a timer length when you are ready",
    "assets__ideas__focus-1.png",
    "Task Planner"
   ],
   [
    "A small session, made visible",
    "An hourglass-inspired reminder that a session can have a clear beginning and end.",
    "Choose your time",
    "One task|The app timer|A comfortable seat|A stopping point",
    "Choose what you will work on|Set a timer length that suits you|Decide what you will do when it ends",
    "assets__ideas__focus-2.png",
    "Task Planner"
   ],
   [
    "Leave a place for the next step",
    "A small finishing ritual to make it easier to return to your task later.",
    "A gentle stopping point",
    "Your task notes|A pen|A place for supplies|A note for next time",
    "Notice where you stopped|Write the next small step|Put the essentials together for later",
    "assets__ideas__focus-4.png",
    "Task Planner"
   ],
   [
    "Listen to a favorite sound",
    "A comfortable seat and a familiar playlist for a small pause.",
    "Your own soundtrack",
    "Headphones or speaker|Music you enjoy|Comfortable seat",
    "Choose something you enjoy|Set a comfortable volume|Pause whenever you want",
    "assets__ideas__extra-season-3.png",
    "Task Planner"
   ],
   [
    "Make a little room to move",
    "A clear corner and a soft mat invite a comfortable movement break.",
    "A gentle pause",
    "Clear space|Optional mat|Water",
    "Choose movements comfortable for you|Go at your own pace|Stop when you want",
    "assets__ideas__extra-season-4.png",
    "Task Planner"
   ],
   [
    "A little autumn journaling",
    "A blank page and a pen, with room for a thought, a doodle or nothing much.",
    "No perfect words",
    "Notebook|Pen|Optional colored pencils|A clear little space",
    "Open a fresh page|Write one thought or draw one shape|Leave the page as it is",
    "assets__ideas__care-3.png",
    "Task Planner"
   ],
   [
    "Save it for another day",
    "A place for ideas you want to keep without acting on them today.",
    "It can wait",
    "Journal or planner|Loose thoughts|A pen|Some room for later",
    "Write the idea down|Note anything you might want to remember|Leave the next step for another day",
    "assets__ideas__brain-4.png",
    "Task Planner"
   ]
  ]
 },
 "October Calendar": {
  "intro": "Choose a seasonal moment and add your own date in Task Planner.",
  "items": [
   [
    "Choose your pumpkin",
    "A visit to a pumpkin patch or a little pumpkin picked up close to home.",
    "One autumn outing",
    "A place to visit|A date that works|A way to carry your pumpkin|An optional companion",
    "Choose where you would like to go|Check opening times before leaving|Give the outing a date",
    "assets__ideas__countdown-1.png",
    "Task Planner"
   ],
   [
    "A night to make something",
    "Set aside a little time for paper bats, a garland or a handmade October detail.",
    "Make a small plan",
    "Paper or card|String|Craft supplies|A clear surface",
    "Choose one small project|Check supplies you already own|Set aside a little making time",
    "assets__ideas__countdown-2.png",
    "Task Planner"
   ],
   [
    "Try the costume together",
    "Give your outfit a simple rehearsal so the details are ready when you need them.",
    "A little preparation",
    "Your outfit|Shoes|Accessories|A mirror",
    "Gather your pieces|Try them together|Write down any small adjustments",
    "assets__ideas__countdown-3.png",
    "Task Planner"
   ],
   [
    "Make room for the night",
    "Choose the few things that will make your Halloween evening feel like yours.",
    "Your kind of Halloween",
    "Your evening plan|A start time|A few essentials|An easy backup idea",
    "Choose how you want to spend the evening|Check the practical details|Save a date and one next step",
    "assets__ideas__countdown-4.png",
    "Task Planner"
   ],
   [
    "Candlelit pumpkin supper",
    "An intimate table with autumn colors and an uncomplicated menu.",
    "Gather around",
    "Pumpkins|Place settings|Battery candles|Simple dinner menu",
    "Set one place for each guest|Arrange pumpkins and leaves with room for serving dishes|Add battery candles clear of plates and glasses",
    "assets__ideas__party-1.png",
    "Task Planner"
   ],
   [
    "Cozy movie night",
    "Cushions, popcorn and one Halloween film everyone can settle into.",
    "A relaxed evening",
    "Film choice|Cushions and blankets|Popcorn|Drinks",
    "Choose a film and test picture and sound|Arrange cushions so everyone can see|Portion popcorn before guests arrive",
    "assets__ideas__party-2.png",
    "Task Planner"
   ],
   [
    "An October game evening",
    "A small board game and a warm drink make room for shared time.",
    "Play a little",
    "A game you own|Comfortable seats|Drinks",
    "Choose a game together|Clear a little table space|Decide how long you want to play",
    "assets__ideas__extra-season-1.png",
    "Task Planner"
   ],
   [
    "An autumn color walk",
    "A short walk for noticing golden leaves, quiet corners and the changing season.",
    "Enjoy what you notice",
    "Comfortable shoes|Weather-appropriate layer|Optional water bottle|A familiar route",
    "Choose a route and check the weather|Go at a pace you enjoy|Notice one color or detail along the way",
    "assets__ideas__care-4.png",
    "Task Planner"
   ],
   [
    "Pumpkin soup night",
    "A warm bowl of pumpkin soup, toasted bread and a few simple toppings.",
    "A cozy supper",
    "Pumpkin or squash|Vegetable stock|Bread|Pumpkin seeds and herbs",
    "Choose a soup recipe and number of servings|Check ingredients and dietary preferences|Make space for preparation and serving",
    "assets__ideas__meals-1.png",
    "Task Planner"
   ],
   [
    "Pack away the pumpkins",
    "Store the decorations you want to see again, grouped so they are easy to find.",
    "A little at a time",
    "Storage box|Tissue or wrapping|Labels|Decorations to keep",
    "Choose one small area|Wrap fragile pieces and group items|Label the box and note its storage spot",
    "assets__ideas__after-1.png",
    "Task Planner"
   ]
  ]
 }
};
function ideaNotes(item){return `Inspired by: ${item[0]}\n\nSupplies to check:\n${item[3].split('|').map(x=>'☐ '+x).join('\n')}\n\nSmall steps:\n${item[4].split('|').map(x=>'☐ '+x).join('\n')}\n\nAlready own / can borrow:\nMy changes:\nOne next step:`;}
function inspirationGallery(mod){const set=inspirationSets[mod];if(!set)return '';return `<section class="inspiration-gallery" aria-labelledby="inspiration-heading"><div class="gallery-heading"><div><span class="studio-eyebrow">A LITTLE INSPIRATION</span><h2 id="inspiration-heading">Ten ways to make it yours.</h2><p>${set.intro}</p></div><span class="gallery-count">${set.items.length} ideas</span></div><div class="idea-grid">${set.items.map((item,i)=>`<article class="idea-card"><button class="idea-image" data-idea-preview="${i}" data-module="${mod}" aria-label="Explore ${esc(item[0])}"><img src="${item[5]}" alt="${esc(item[0]+'. '+item[1])}" loading="lazy" width="768" height="512"><span>Explore idea ↗</span></button><div class="idea-copy"><span class="idea-tag">${item[2]}</span><h3>${item[0]}</h3><p>${item[1]}</p><button class="idea-use" data-studio-step="${esc(item[0])}" data-module="${item[6]}" data-notes="${esc(ideaNotes(item))}">Use this idea <span aria-hidden="true">＋</span></button></div></article>`).join('')}</div><p class="gallery-note">Ideas to adapt, not a to-do list. Choose one, edit the details and save when you are ready.</p></section>`;}
document.addEventListener('click',e=>{const b=e.target.closest('[data-idea-preview]');if(!b)return;const mod=b.dataset.module,set=inspirationSets[mod],i=Number(b.dataset.ideaPreview),item=set?.items[i];if(!item)return;showDialog(item[0],`<div class="idea-detail"><img src="${item[5]}" alt="${esc(item[1])}"><p>${item[1]}</p><h3>Supplies to check</h3><ul>${item[3].split('|').map(x=>`<li>${x}</li>`).join('')}</ul><h3>Make a small start</h3><ol>${item[4].split('|').map(x=>`<li>${x}</li>`).join('')}</ol><button class="primary" data-studio-step="${esc(item[0])}" data-module="${item[6]}" data-notes="${esc(ideaNotes(item))}">Use this idea</button><p class="gallery-note">This opens an editable plan. Nothing is saved until you choose Save entry.</p></div>`);});

function supplementalGallery(mod){const html=inspirationGallery(mod);return html?`<details class="focus-inspiration"><summary>Explore 10 ${mod==='Budget'?'spending':mod==='Overwhelmed Mode'?'gentle':'seasonal'} ideas <span>Optional inspiration</span></summary>${html}</details>`:'';}
